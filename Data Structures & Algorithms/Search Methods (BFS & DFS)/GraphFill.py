#  File: GraphFill.py
#  Description:
#  Student Name: Antonio Gonzalez
#  Student UT EID: ag72672
#  Partner Name: Juan Chavarria
#  Partner UT EID: jac23763
#  Course Name: CS 313E
#  Unique Number:
...
class ImageGraph:
    ...
    def print_adjacency_matrix(self):
        ...
        # makes all-zero matrix first
        i = len(self.nodes)
        matrix = [[0 for j in range(i)] for j in range(i)]   
    ...
    def bfs(self, start_index, color):
        ...
        while q.is_empty() == False: 
            ...
            curr = q.peek()
            # Only print image if the node hasn't been changed
            if curr.visited == False:
                curr.visit_and_set_color(color)
                self.print_image()
            
            # Traverse edge matrix of node and enqueue any edges of the same
            # color if they haven't been visited yet
            for edge in curr.edges:
                if (self.nodes[edge].color == og_color 
                and self.nodes[edge].visited == False):
                    q.enqueue(self.nodes[edge])
            
            # Remove current pixel once all its info has been extracted
            q.dequeue()
    ...
    def dfs(self, start_index, color):
        ...
        while s.is_empty() == False:
            ...
            # Traverse all node edges until the first edge with the same color
            # that hasn't been visited shows up
            for edge in curr.edges:
                if (self.nodes[edge].color == og_color 
                and self.nodes[edge].visited == False):
                    s.push(self.nodes[edge])
                    break
                # Pop current item from stack if no edge values were the same 
                # color or if the edge had been visited already
                if curr.edges.index(edge) == (len(curr.edges)-1):
                    s.pop()
